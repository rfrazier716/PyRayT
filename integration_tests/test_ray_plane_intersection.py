...
class TestOpticsTrainRefraction(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.results = tracer.trace()

    def test_result_dataframe(self):
        # The results length should be 150
        self.assertEqual(len(self.results), 150)

        # generation 2 should be effectively parallel along the x-axis
        x_vec = self.results.loc[self.results["generation"] == 2]["x1"]
        self.assertTrue(np.allclose(x_vec, 1.0))
