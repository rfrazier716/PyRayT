...
def wavelength_to_rgb(wavelength: Tuple[float, np.ndarray], gamma=0.8) -> np.ndarray:
    ...
    red = np.abs((-(clipped_wave - zone_max) / (zone_max - zone_min) * attenuation )) ** gamma
    green = np.full(wavelength.shape, 0.0)
    blue = np.abs((1.0 * attenuation)) ** gamma
    zone = np.vstack((red, green, blue)) # the color calculations based on the first zone
    color = np.where(wavelength < zone_max, zone, color) # fill the color array at the appropriate spots
    ...
    zone = np.vstack((red, green, blue)) # the color calculations based on the first zone
    color = np.where(np.logical_and(wavelength >= zone_min, wavelength < zone_max), zone, color) # fill the color array at the appropriate spots
    ...
    zone = np.zeros((3, wavelength.shape[0])) # the color calculations based on the first zone
